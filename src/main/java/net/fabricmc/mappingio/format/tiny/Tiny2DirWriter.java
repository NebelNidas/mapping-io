...
public final class Tiny2DirWriter implements MappingWriter {
	public Tiny2DirWriter(Path dir, boolean escapeNames, boolean deleteExistingFiles) throws IOException {
		...
		if (deleteExistingFiles && Files.exists(dir)) {
			Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
				...
				public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
					if (file.getFileName().toString().endsWith("." + MappingFormat.TINY_2_FILE.fileExt)) {
						Files.delete(file);
					}
